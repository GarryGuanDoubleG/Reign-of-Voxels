...
void VoxelOctree::InitializeOctree(sf::Image *heightmap, int world_side)
{
	...
	m_boundingRegion = new VoxelChunk(m_chunkCount++, Vec3(0.0f, 0.0f, 0.0f));
...
{
	int size = m_boundingRegion->getSize();
	Vec3 pos = m_boundingRegion->getPosition();

	int child_radius = m_boundingRegion->getSize() / 2;
	int i = 0;
	vox_count++;
	...
	for (float x = 0; x < 2; x++)
	{
		for (float y = 0; y < 2; y++)
		{
			for (float z = 0; z < 2; z++)
			{
				...
				m_childNodes[i]->m_boundingRegion = new VoxelChunk(m_chunkCount++, chunk_pos);
