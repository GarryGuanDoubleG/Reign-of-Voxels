...
void AnimationController::LoadAnimation(const std::string &filepath, const std::string &animName)
{
	...
	assimpToGLMMat4(&m_scenes[m_numAnims]->mRootNode->mTransformation.Inverse(), globalInverseTrans);
	m_GlobalInverseTransforms.push_back(globalInverseTrans);

	m_animIDMap.insert(std::pair<std::string, int>(animName, m_numAnims));

	m_numAnims++;
}


void AnimationController::BoneTransform(float timeInSecs)
{
...
const aiNodeAnim* AnimationController::FindNodeAnim(const aiAnimation* anim, const std::string NodeName)
{
...
void AnimationController::InterpolateScaling(aiVector3D &out, float animTime, const aiNodeAnim *animNode)
{
...
void AnimationController::InterpolateRotation(aiQuaternion & out, float animTime, const aiNodeAnim* animNode)
{
...
void AnimationController::InterpolatePosition(aiVector3D& out, float animTime, const aiNodeAnim* animNode)
{
...
void AnimationController::CalculateHeiarchyTransform(float animTime, aiNode *node, glm::mat4 parentTransform)
{
	...
	{
		assimpToGLMMat4(&node->mTransformation, nodeTransform);
	}
...
void AnimationController::SetAnimation(float currTime, const std::string &name)
{
	m_playTime = currTime;
